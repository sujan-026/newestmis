...
//------------------------------------------------------------------------------------------  
        ...
        const [selectedBranchCurrent, setSelectedBranchCurrent] = useState<string>("");
        const [selectedBranchPrevious, setSelectedBranchPrevious] = useState<string>("");
...
const exportToExcel = () => {
    ...
    xlsx.writeFile(wb, 'Change_of_Branch.xlsx');
};
//-------------------------------------------------------------------------------------------------------------------
        useEffect(() => {
...
//-----------------------calling fetch data api---------------------------------------------------------
        ...
        const fetchAdmissions = async () => {
            setLoading(true);
            try {
                const response = await fetch(`/api/adm_cob_usno_fetch?admission_year=${admissionYear}`);
                if (!response.ok) {
...
//---------------------------------------------------------------------------------------------------------------------
        const handleRowClick = (admission: Admission) => {
            ...
            setSelectedBranchCurrent(admission.course|| '');
            setSelectedBranchPrevious(admission.old_brcode|| '');
...
//-------------------------------------------------------------------------------------------------------------------------
        const handleBranchChangeCurrent = (course: string) => {
            setSelectedBranchCurrent(course);
        ...
        const handleBranchChangePrevious = (old_brcode: string) => {
            setSelectedBranchPrevious(old_brcode);
            if (selectedAdmission) {
                setSelectedAdmission((prev) => {
                    if (!prev) return prev;
                    return {
...
//------------------------------------Form Submit--------------------------------------------------            
        const handleFormSubmit = async (e: React.FormEvent) => {
            ...
            if (selectedAdmission) {
                try {
                const response = await fetch(`/api/adm_cob_branch_update`, {
                    method: 'POST',
        ...
        return (
            ...
            {/* {user && (
                ...
                <div className="ml-2 mt-8">
                    ...
                    <h1 className='ml-4 text-2xl'>Change of Branch USNO and Branch Update</h1>
                                ...
                                <table className="min-w-full bg-white border border-gray-300">
                                    <thead className="bg-gray-200">
                                        <tr>
                                            ...
                                            <th className="px-4 py-2 border">Old Branch</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {filteredData.map((admission, index) => (
                                            ...
                                            <tr 
                                                ...
                                                <td className="border px-4 py-2">{admission.old_brcode}</td>
                                            </tr>
                                            )
                                        )}
                                    </tbody>
                            ...
                            <div className=" max-w-6xl bg-red-50 shadow-md rounded-md  my-2 mt-2">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="rounded w-96 px-2 py-1 cursor-pointer">
                                        ...
                                        <BranchSelect selectedBranch={selectedBranchPrevious} handleChange={handleBranchChangePrevious} />
                                        <h2>Selected Branch: {selectedBranchPrevious}</h2>
                                    </div>
                                    {/* Course*/}
                                    <div className="rounded w-96 px-2 py-1 cursor-pointer">
                                        <label
                                            ...
                                            htmlFor="cource"
                                        >
                                            Change of Branch Opted 
                                        </label>
                                        <BranchSelect selectedBranch={selectedBranchCurrent} handleChange={handleBranchChangeCurrent} />
                                        <h2>Selected Branch: {selectedBranchCurrent}</h2>
                                    </div>
                                </div>    
                            </div>
                            <button
